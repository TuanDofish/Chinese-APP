...
class _DictionarySearchTabState extends State<DictionarySearchTab> {
  ...
  bool _isBrokenText(String text) =>
      RegExp(r'[ÃƒÃ„Ã‚Ã°Å¸]|ï¿½').hasMatch(text) ||
  ...
  Future<void> _fetchAutocomplete(String q) async {
    try {
      final uri = Uri.parse(
        '$_apiBase/dictionary/autocomplete?q=${Uri.encodeComponent(q)}',
      );
      ...
      if (mounted) {
        setState(() {
          ...
          if (_searchResult == null) {
            ...
            _errorMessage =
                'Không tìm thấy trong API, đang hiển thị dữ liệu tối giản.';
          }
        });
      }
    } catch (e) {
      ...
      if (mounted) {
        setState(() {
          ...
          if (bundled != null) {
            _searchResult = bundled;
            _errorMessage =
                'Đang offline: hiển thị từ dữ liệu có sẵn trên app.';
          } else {
  ...
  Future<Map<String, dynamic>?> _searchLocalDB(String q) async {
    try {
      final uri = Uri.parse(
        '$_apiBase/dictionary/search?q=${Uri.encodeComponent(q)}',
      );
      final res = await http.get(uri).timeout(const Duration(seconds: 4));
      if (res.statusCode == 200) {
        final List<dynamic> list = json.decode(res.body);
  ...
  Map<String, dynamic> _normalizeVocab(Map<String, dynamic> item) {
    ...
    final simplified = TextSanitizer.clean(
      (item['simplified'] ?? '').toString(),
    );
    final traditional = TextSanitizer.clean(
      (item['traditional'] ?? '').toString(),
    );
    meaning = TextSanitizer.clean(meaning);
    pinyin = TextSanitizer.clean(pinyin);

    if (_isBrokenText(meaning) ||
        _isBrokenText(pinyin) ||
        meaning.trim().isEmpty) {
      final local = VocabDataHelper.getData(simplified, {
    ...
    return {
      ...
      'pinyin': pinyin,
      'meaning': meaning,
      'hanViet': TextSanitizer.clean(
        (item['hanViet'] ?? item['han_viet'] ?? '').toString(),
      ),
      'radical': TextSanitizer.clean((item['radical'] ?? '').toString()),
      'wordType': TextSanitizer.clean(
        (item['wordType'] ?? item['word_type'] ?? '').toString(),
      ),
      ...
      children: [
        ...
        if (_isSearching)
          const Padding(
            ...
            child: Center(
              child: CircularProgressIndicator(color: Color(0xFFC63D33)),
            ),
          ),
    ...
    return Container(
      ...
      child: Column(
        children: _suggestions.asMap().entries.map((entry) {
          ...
          final py = PinyinUtils.convertSpaced(
            TextSanitizer.clean(s['pinyin'] as String? ?? ''),
          );
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                border: isLast
                    ? null
                    : const Border(
                        bottom: BorderSide(color: Color(0xFFF5F0EC), width: 1),
                      ),
              ),
              child: Row(
                children: [
