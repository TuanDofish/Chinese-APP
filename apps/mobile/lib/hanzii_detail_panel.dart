...
class _HanziiDetailPanelState extends State<HanziiDetailPanel> {
  ...
  void initState() {
    ...
    if (_data['meaning'] == 'Đang tải...' || (_data['examples'] as List).isEmpty) {
      VocabDataHelper.getDataAsync(_data['simplified'], widget.wordData)
          .then((d) {
  ...
  void _toggleFavorite() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(v ? 'Đã lưu "$word" vào Sổ tay ⭐' : 'Đã xóa "$word" khỏi Sổ tay'),
  ...
  Future<void> _checkPronunciation() async {
    ...
    setState(() { _isListening = true; _pronScore = null; _recognized = ''; });
    final result = await SpeechService.listen();
    if (mounted) {
      final score = SpeechService.calculateScore(_data['simplified'], result);
      setState(() { _isListening = false; _recognized = result; _pronScore = score; });
    }
  }

  Color _scoreColor(int s) => s >= 80 ? _jade : s >= 50 ? _amber : _cinnabar;
  String _scoreEmoji(int s) =>
      s >= 90 ? '🎉 Xuất sắc!' : s >= 70 ? '👍 Tốt lắm!' : s >= 50 ? '😊 Khá ổn!' : '💪 Thử lại!';

  @override
  Widget build(BuildContext context) {
    final simplified = _data['simplified'] as String? ?? '';
    final pinyinRaw = _data['pinyin'] as String? ?? '';
    final pinyin = PinyinUtils.convertSpaced(pinyinRaw); // "you2 yong3" → "yóu yǒng"
    ...
    return DefaultTabController(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            // ─── Header ──────────────────────────────────────────────────────────
            _buildHeader(simplified, pinyin, hanViet, wordType, hskLevel, meaning),
            ...
            Expanded(
              child: TabBarView(
                children: [
                  _buildDictionaryTab(simplified, meaning, wordType, radical, hskLevel, examples),
  ...
  // ─── Header ──────────────────────────────────────────────────────────────
  Widget _buildHeader(String simplified, String pinyin, String hanViet,
      String wordType, dynamic hskLevel, String meaning) {
        ...
        children: [
          Row(
            ...
            children: [
                      ...
                      children: [
                        ...
                        if (pinyin.isNotEmpty)
                          Padding(
                            ...
                            child: Text(pinyin,
                                style: const TextStyle(
                                    fontSize: 26, color: _cinnabar, fontWeight: FontWeight.w600)),
                          ),
              ...
              Column(
                children: [
                  _iconBtn(Icons.volume_up_rounded, _amber, () => _speak(simplified),
  ...
  // ─── Tab 1: Từ điển ─────────────────────────────────────────────────────
  Widget _buildDictionaryTab(String simplified, String meaning, String wordType,
      String radical, dynamic hskLevel, List<Map<String, dynamic>> examples) {
    return ListView(
      ...
      children: [
        ...
        _sectionCard(
          child: Column(
            ...
            children: [
              Row(children: [
                const Icon(Icons.translate_rounded, size: 16, color: _cinnabar),
        ...
        if (examples.isNotEmpty) ...[
          _sectionCard(
            child: Column(
              ...
              children: [
                Row(children: [
                  const Icon(Icons.format_quote_rounded, size: 16, color: _amber),
                  const SizedBox(width: 6),
                ...
                ...examples.asMap().entries.map((e) =>
                    _buildExampleItem(e.key + 1, e.value, simplified)),
              ],
            ),
          ),
          const SizedBox(height: 12),
        ...
        if (radical.isNotEmpty)
          _sectionCard(
            child: _buildMetaItem('Bộ thủ', radical, Icons.category_rounded, _cinnabar),
          ),
      ],
    );
  }

  Widget _buildExampleItem(int num, Map<String, dynamic> ex, String simplified) {
    return Padding(
          ...
          child: Row(
            ...
            children: [
              Container(
                ...
                child: Text('$num',
                    style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: _cinnabar)),
              ),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    ...
                    Text(PinyinUtils.convert(ex['py'] ?? ''),
                        style: const TextStyle(fontSize: 13, color: _cinnabar)),
                    const SizedBox(height: 3),
                    Text(ex['vi'] ?? '',
                        style: const TextStyle(fontSize: 14, color: _muted, fontStyle: FontStyle.italic)),
              ...
              const Icon(Icons.volume_up_outlined, size: 18, color: Color(0xFFE7DDD0)),
            ],
          ),
        ),
      ),
  ...
  Widget _highlightHanzi(String text, String highlight) {
    if (highlight.isEmpty || !text.contains(highlight)) {
      return Text(text, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w600, color: _ink));
    }
    ...
    for (int i = 0; i < parts.length; i++) {
      ...
      if (i < parts.length - 1) {
        spans.add(TextSpan(
          text: highlight,
          style: const TextStyle(color: _cinnabar, fontWeight: FontWeight.w800),
        ));
      }
    }
    return RichText(
  ...
  Widget _buildMetaItem(String label, String value, IconData icon, Color color) {
    return Expanded(
      child: Row(
        children: [
  ...
  // ─── Tab 2: Hán tự ──────────────────────────────────────────────────────
  Widget _buildCharacterTab(String simplified, String pinyin, String hanViet, String radical) {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
            ...
            children: [
              Container(
                ...
                child: Text(simplified,
                    style: const TextStyle(fontSize: 64, fontWeight: FontWeight.w900, color: _cinnabar)),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _charInfo('Pinyin', pinyin.isNotEmpty ? pinyin : '—', _cinnabar),
                    const SizedBox(height: 10),
                    _charInfo('Hán Việt', hanViet.isNotEmpty ? hanViet : '—', _jade),
                    const SizedBox(height: 10),
                    _charInfo('Bộ thủ', radical.isNotEmpty ? radical : '—', _amber),
                    const SizedBox(height: 10),
                    _charInfo('Số nét', widget.wordData['strokeCount'] != null && widget.wordData['strokeCount'] != 0
            ...
            children: [
              ...
              SizedBox(
                ...
                child: Row(
                  ...
                  children: List.generate(5, (i) => _strokeStep(simplified, i + 1)),
                ),
              ),
              const SizedBox(height: 10),
              const Center(
                child: Text('(Tính năng hoạt ảnh nét vẽ đang phát triển)',
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
                color: step == 1 ? _cinnabar.withValues(alpha: 0.4) : const Color(0xFFE7DDD0)),
          ),
          child: Text(char,
              style: TextStyle(
                  fontSize: 28,
      ...
      children: [
        _sectionCard(
          child: Column(
            children: [
              Text(simplified,
                  style: const TextStyle(fontSize: 64, fontWeight: FontWeight.w900, color: _ink)),
                  ...
                  _isListening
                      ? Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(color: _cinnabar.withValues(alpha: 0.3)),
                          ),
                          child: const Row(
                            mainAxisAlignment: MainAxisAlignment.center,
                            children: [
                              SizedBox(width: 18, height: 18,
                                  child: CircularProgressIndicator(strokeWidth: 2, color: _cinnabar)),
                              SizedBox(width: 8),
                              Text('Đang nghe...', style: TextStyle(fontSize: 12, color: _cinnabar)),
                            ],
                          ),
                        )
                      : _actionButton(
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    const Text('/100', style: TextStyle(fontSize: 18, color: _muted)),
                  ],
                ),
                Text(_scoreEmoji(_pronScore!), style: const TextStyle(fontSize: 18)),
