...
import 'pronunciation_data.dart';
...
class _NewsReaderScreenState extends State<NewsReaderScreen>
  ...
  Future<void> _analyzePronounciation() async {
    ...
    final score =
        _calculateSimilarity(_currentLesson.chinese, _recognizedText);
    if (mounted) setState(() {
  ...
  Map<String, dynamic> _buildScoreResult(double score) {
    final s = score.round();
    if (s >= 92) {
      return {'score': s, 'grade': "Xuất sắc! 🏆", 'color': Colors.green,
        'feedback': "Phát âm của bạn rất chuẩn! Tuyệt vời!", 'tips': []};
    } else if (s >= 75) {
      return {'score': s, 'grade': "Tốt lắm! 🌟", 'color': Colors.blue,
        'feedback': "Phát âm khá tốt, chú ý một vài âm tiết nhỏ.",
        'tips': ["Thử đọc lại chậm hơn, chú ý từng thanh điệu."]};
    } else if (s >= 55) {
      return {'score': s, 'grade': "Khá ổn 👍", 'color': Colors.orange,
        'feedback': "Cần luyện tập thêm. Nghe mẫu nhiều lần rồi đọc theo.",
        'tips': ["Nghe mẫu (nút loa) 3-5 lần trước khi tự đọc.", "Tập từng từ trước rồi mới ghép câu."]};
    } else if (s >= 35) {
      return {'score': s, 'grade': "Cần cố gắng 💪", 'color': Colors.deepOrange,
        'feedback': "Phát âm cần cải thiện nhiều. Đừng nản lòng!",
        'tips': ["Bắt đầu với câu ngắn hơn ở cấp độ dưới.", "4 thanh điệu: bằng(ā), lên(á), xuống-lên(ǎ), xuống(à)."]};
    } else {
      return {'score': s, 'grade': "Hãy thử lại! 🎯", 'color': Colors.red,
        'feedback': "Máy chưa nhận ra bạn đọc. Đọc to hơn và rõ hơn nhé!",
        'tips': ["Đảm bảo không có tiếng ồn xung quanh.", "Đọc to, rõ ràng ngay gần micro điện thoại."]};
    }
  }
  ...
  Future<void> _fetchLiveNews() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final articles = _parseRSS(body, source['name']!, source['level']!, colors);
        setState(() {
          _liveArticles = articles;
          _isLoadingNews = false;
          if (articles.isNotEmpty) _selectedLiveArticle = articles[0];
          if (articles.isEmpty) {
            _newsError = 'Tải thành công nhưng không có bài đọc tiếng Trung. Thử nguồn khác.';
          }
        });
      } else {
        setState(() {
          _isLoadingNews = false;
          _newsError = 'Server trả lỗi (HTTP ${response.statusCode}). Thử nguồn khác hoặc thử lại sau.';
        });
      }
    } catch (e) {
      setState(() {
        _isLoadingNews = false;
        _newsError = 'Lỗi kết nối: Không tải được tin tức.\nKiểm tra mạng và thử lại.\n(${e.toString().split(':').first})';
      });
    }
  }

  List<NewsArticle> _parseRSS(
      String xml, String sourceName, String level, List<Color> colors) {
    ...
    for (final m in itemRegex.allMatches(xml)) {
      ...
      bool hasChinese = RegExp(r'[\u4e00-\u9fff]').hasMatch(title + description);
      ...
      articles.add(NewsArticle(
        title: title.isNotEmpty ? title : description.substring(0, description.length.clamp(0, 50)),
        titleVi: '[$sourceName]',
        content: content,
        source: sourceName,
        level: level,
  ...
  String _extractTag(String xml, String tag) {
    final r1 = RegExp('<$tag[^>]*><!\\[CDATA\\[(.+?)\\]\\]><\\/$tag>', dotAll: true);
  ...
  Future<void> _lookupWord(String word) async {
    ...
    setState(() {
      _wordDefinition = {
        'word': word,
        'pinyin': '',
        'meaning': meaning.isNotEmpty ? meaning : '(Không tra được nghĩa lúc này)',
      };
      _isLoadingDefinition = false;
    });
  }
  ...
  bool _isCommonWord(String w) {
    const s = {
      '今天','天气','非常','公园','跑步','很多','新闻','中国','经济','发展',
      '政府','社会','文化','教育','科技','环境','国际','合作','报道','记者',
      '事件','影响','重要','问题','解决','国家','政策','改革','基础','设施',
      '研究','数据','人工','智能','互联','网络','技术','创新','企业','市场',
      '消费','贸易','投资','金融','银行','货币','汇率','价格','通货','膨胀',
      '学习','语言','方法','首先','其次','每天','练习','汉字','歌曲','电影',
      '喜欢','朋友','认识','中文','越南','周末','常常','一起','吃饭','好吃',
      '传统','节日','春节','鞭炮','饺子','孩子','红包','中秋','月饼','端午',
      '粽子','龙舟','不仅','庆祝','团聚','生活','现在','人们','支付','外卖',
      '面对','交流','思考','需要','改变','各个','医疗','气候','能源','信息',
    };
    return s.contains(w);
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text("${_currentLessonIndex + 1} / $totalCount câu",
            ...
            child: Column(
              children: [
                ...
                Text(lesson.pinyin,
                    ...
                    textAlign: TextAlign.center),
                const SizedBox(height: 4),
                Text(lesson.vietnamese,
                    style: const TextStyle(fontSize: 13, color: Colors.white60),
                    textAlign: TextAlign.center),
                const SizedBox(height: 16),
                OutlinedButton.icon(
                  icon: Icon(_ttsSpeaking ? Icons.stop : Icons.volume_up,
                      color: Colors.white),
                  label: Text(_ttsSpeaking ? "Dừng" : "Nghe mẫu phát âm",
          ...
          Center(
            child: Column(
              children: [
                const Text("Bấm mic và đọc câu bên trên:",
                ...
                Text(
                  _isListening ? "🎙️ Đang nghe... (Bấm để dừng)" : "Bấm để bắt đầu đọc",
                  style: TextStyle(
    ...
    return Column(
      children: [
        Container(
          ...
          child: Row(
            children: [
              const Text("Nguồn: ", style: TextStyle(fontWeight: FontWeight.w600)),
              Expanded(
            ...
            child: Center(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  const Text("Nhấn để tải tin tức mới nhất",
                      style: TextStyle(color: Colors.grey)),
                  const SizedBox(height: 20),
                  ElevatedButton.icon(
            ...
            child: Column(
              children: [
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(
                                color: isSelected ? a.color : Colors.grey.shade200,
                ...
                if (_selectedLiveArticle != null)
                  Expanded(
                    child: _buildArticleContent(_selectedLiveArticle!),
                  ),
  ...
  Widget _buildArticleContent(NewsArticle article) {
    final words = _splitText(article.content);
    return SingleChildScrollView(
      padding: const EdgeInsets.all(16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Row(
            children: [
              ...
              const Spacer(),
              IconButton(
                icon: const Icon(Icons.volume_up, color: Color(0xFFD32F2F)),
                    ...
                    child: Container(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          fontSize: 22,
                          height: 1.8,
                          color: isChinese ? Colors.black87 : Colors.grey,
          ...
          : Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.center,
                  children: [
                    Text(_wordDefinition['word'] ?? '',
                        style: const TextStyle(
                            fontSize: 36,
                            fontWeight: FontWeight.bold,
                            color: Color(0xFFD32F2F))),
                    if ((_wordDefinition['pinyin'] ?? '').isNotEmpty)
                      Text(_wordDefinition['pinyin']!,
                          style: const TextStyle(
                              fontSize: 14,
                              color: Colors.blue,
                              fontStyle: FontStyle.italic)),
                ...
                Column(
                  children: [
                    IconButton(
                      icon: const Icon(Icons.volume_up, color: Color(0xFFFFC107)),
                      onPressed: () => _tts.speak(_wordDefinition['word'] ?? ''),
                    ),
                    IconButton(
                      icon: const Icon(Icons.bookmark_add, color: Color(0xFFD32F2F)),
                      onPressed: () async {
                        await _progressService.toggleFavorite(_wordDefinition['word'] ?? '');
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            content: Text('Đã lưu "${_wordDefinition['word']}" ⭐'),
