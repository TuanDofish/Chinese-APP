...
class _QuizScreenState extends State<QuizScreen> with TickerProviderStateMixin {
  ...
  void _buildQuestions() {
    ...
    final fallbacks = ['的', '是', '有', '在', '好', '我', '你', '他', '来', '去', '大', '小'];

    for (final word in shuffled) {
      // Pick 3 wrong options
      final others = allData.where((w) => w['simplified'] != word['simplified']).toList()..shuffle(_random);
      final wrongOptions = others.take(3).map((w) => w['simplified'] as String).toList();
      ...
      final options = [word['simplified'] as String, ...wrongOptions]..shuffle(_random);
                      ...
                      child: Row(
                        children: [
                          Image.asset(
                            ...
                            height: 80,
                            errorBuilder: (_, __, ___) => const _PandaFallback(),
                          ),
                          ...
                          AnimatedOpacity(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: _isCorrect ? _kGreen.withOpacity(0.12) : _kRed.withOpacity(0.12),
                                borderRadius: BorderRadius.circular(12),
                                border: Border.all(
                                  color: _isCorrect ? _kGreen.withOpacity(0.3) : _kRed.withOpacity(0.3),
                                ),
                              ),
                              child: Text(
                    ...
                    Expanded(
                      child: GridView.count(
                        ...
                        children: q.options.map((opt) {
                          final optData = VocabDataHelper.getData(opt, {'simplified': opt, 'forms': []});
                          final optPy = PinyinUtils.convertSpaced(optData['pinyin'] as String? ?? '');
                          return _AnswerTile(
                            hanzi: opt,
                            pinyin: optPy,
                            state: _isAnswered
                            ...
                            shakeAnim: opt == _selectedAnswer && !_isCorrect ? _shakeAnim : null,
                          );
                        }).toList(),
                      ),
                    ),
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  foregroundColor: _isAnswered ? Colors.white : Colors.grey.shade400,
                  elevation: 0,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                ),
                child: Text(
                  _isAnswered
                      ? (_currentIndex >= _questions.length - 1 ? 'Hoàn thành 🎉' : 'Bước tiếp theo')
                      : 'Chọn đáp án',
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(slow ? Icons.slow_motion_video_rounded : Icons.volume_up_rounded, color: color, size: 30),
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            height: 100,
            decoration: BoxDecoration(
              color: isGood ? _kGreen.withOpacity(0.1) : _kAmber.withOpacity(0.1),
              shape: BoxShape.circle,
              border: Border.all(
